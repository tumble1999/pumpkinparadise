...
$(".login form").submit(function(e) {
	...
	if (username.length < 4 || username.length > 32) {
		$("#username")
			.feedback(errorMessages.nickname)
			.focus();
		isValid = false;
	} else {
		$("#username").feedback();
	...
	if (isValid) {
		...
		document.getElementById("pumpkinlevel").innerHTML = localStorage.getItem(
			"level"
		);
	}
});
	...
	if (result !== null) {
		...
		if (error.error == "EmailAddressNotAvailable") {
			$(".join .form-error")
				...
				.show();
			$("#joinEmail")
				.feedback("Email address is not available")
				.focus();
		} else {
		...
		if (error.error == "NameNotAvailable") {
			$(".join .form-error")
				.text("Nickname is not available")
				.show();
			$("#joinNickname")
				.feedback("Nickname is not available")
				.focus();
		}
	}
};
